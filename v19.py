...
txt="屏幕排布完成后，采集界面点击开始采集，后台界面勾选通道1、2，\n勾选“启动劫持”，选择对应试验类型，最后点击 1预备"
time_0=500
time_1=10
# # ===== HEX / ASCII 打印 =====
...
def forward(src, dst, direction):
    global real_dat, out_dat, set_values, set_mult, add_flag_1, add_flag_2,down_p,txt,time_0,time_1,add_flag_3
    try:
        while True:
            data = src.recv(BUFFER_SIZE)
            if not data:
                break
            # print(src, dst, direction)

            mutable = bytearray(data)
            print(direction,mutable)
            hex_str = ' '.join(f'{b:02x}' for b in mutable)
            print(direction,hex_str)
            # 替换 IP
            ...
            # 修改寄存器
                ...
                # 0 预备阶段
                    ...
                    txt="请在电机界面开始采集，并正转启动，开始录屏，最后切换到 2低速"
                ...
                # 3 高速阶段
                    ...
                    # 设置压力
                    set_random[1] = 6
                    if out_dat[0] <= 1320 and add_flag_2:
                        # 设置压力
                        set_random[1] = 12
                        if add_flag_1:
                            if out_dat[1] > 320:
                        ...
                        if add_flag_3:
                            set_add[1] = random.uniform(70, 90) * rate_p*2
                            if out_dat[1] >= max_p * 1000 - 400:
                                out_dat[1] = max_p * 1000 - 400
                                add_flag_3 = False
                        else:
                            set_add[1] = random.uniform(down_p * -2, 0) * rate_p
                        print(add_flag_1,add_flag_2,add_flag_3)
                        # if out_dat[1] < 4000:
                        #     set_add[1] = random.uniform(-12, 4)
                        # else:
                        #     set_add[1] = -40
                    if out_dat[0]>1325 or add_flag_2==False:
                        if time_0 > 0:
                            time_0=time_0-1
                            txt="开始计时 ("+str(time_0)+"s)"
                        else:
                            txt ="请切换至 4泄压"
                    else: txt =f"等待温度上升至1320。。。"
                ...
                # 降温阶段
                    ...
                    if out_dat[1]>600:
                        ...
                        if time_1:
                            time_1=time_1-1
                            txt = f"请减速停机，倒计时({time_1}s)后停止录屏。。。"
                        else: txt = "请停止录屏，随后在转速界面、采集界面点击停止采集"
...
def change_state(new_state):
    global state, add_flag_1,add_flag_2,time_0,time_1,add_flag_3
    if 0 <= new_state <= 4:
        state = new_state
    # 更新状态显示
